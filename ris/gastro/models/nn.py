...
def get_indexed_data(df):
    X, word_dict = bedrock.index_tokenized_sentences(
        df[bedrock.PROCESSED_TOKENS]
    )
    y = df[bedrock.BINARY_CLASS]
...
def get_indexed_and_padded_data():
    ...
    df_processed = bedrock.process_df(
        df,
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        ...
        test_size=0.3,
        random_state=42,
        stratify=df_processed[bedrock.BINARY_CLASS]
    )
...
if __name__ == '__main__':
    '''This example demonstrates the use of Convolution1D for bedrock classification.
