...
from labeloh.model import TrainedModel
from labeloh.project.api.machine_learning.algorithm_selection import functions_dict
...
class All(Resource):
    ...
    def post(self):
        ...
        storage_uri = storage_uri.split('/')[-1] # TODO: fix db entries and remove this

        r_storage = requests.get(storage_api + '/' + storage_uri)
        dataset = r_storage.content
        csv = StringIO(dataset.decode('utf-8'))
        df = pd.read_csv(csv)

        func = functions_dict[algorithm_id]
